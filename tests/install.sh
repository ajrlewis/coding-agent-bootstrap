...
[ -f "$TEST_REPO/.agents/DOCTOR.md" ] || fail "DOCTOR.md was not installed"
[ -f "$TEST_REPO/.agents/todos/TODO.md" ] || fail "active TODO file was not installed"
[ -f "$TEST_REPO/.agents/todos/DONE.md" ] || fail "completed TODO archive was not installed"
[ ! -e "$TEST_REPO/.agents/TODO.md" ] || fail "legacy TODO path was installed"
[ -f "$TEST_REPO/.agents/presets/architecture/monorepo.md" ] || fail "monorepo preset was not installed"
[ -f "$TEST_REPO/.agents/presets/git/azure-devops.md" ] || fail "Azure DevOps preset was not installed"
[ -f "$TEST_REPO/.agents/presets/ci/azure-pipelines.md" ] || fail "Azure Pipelines preset was not installed"
[ -f "$TEST_REPO/.agents/presets/database/sql-server.md" ] || fail "SQL Server preset was not installed"
[ -f "$TEST_REPO/.agents/presets/database/snowflake.md" ] || fail "Snowflake preset was not installed"
[ -f "$TEST_REPO/.agents/presets/infrastructure/bicep.md" ] || fail "Bicep preset was not installed"
[ -f "$TEST_REPO/.agents/presets/observability/logging.md" ] || fail "logging preset was not installed"
[ -f "$TEST_REPO/.agents/presets/quality/api-documentation.md" ] || fail "API documentation preset was not installed"
[ -f "$TEST_REPO/.agents/presets/quality/testing.md" ] || fail "testing preset was not installed"
[ -f "$TEST_REPO/.agents/presets/tooling/turborepo.md" ] || fail "Turborepo preset was not installed"
[ -f "$TEST_REPO/.agents/mcp/doppler.md" ] || fail "Doppler MCP capability was not installed"
...
[ "$(sed -n '1p' "$TEST_REPO/.agents/VERSION")" = "4" ] || fail "payload version is not 4"
cmp "$ROOT_DIR/bootstrap/AGENTS.md" "$TEST_REPO/AGENTS.md" >/dev/null || fail "installed AGENTS.md does not match payload"
grep -F '.agents/DOCTOR.md' "$TEST_REPO/AGENTS.md" >/dev/null || fail "AGENTS.md does not route agent-context maintenance"
grep -F 'Remove this paragraph from `AGENTS.md` when bootstrap is complete.' "$TEST_REPO/AGENTS.md" >/dev/null || fail "AGENTS.md omits bootstrap route cleanup"
grep -F 'Remove the paragraph beginning `If .agents/BOOTSTRAP.md exists`' "$TEST_REPO/.agents/BOOTSTRAP.md" >/dev/null || fail "BOOTSTRAP.md omits AGENTS.md cleanup"
