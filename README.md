...
Repository CI runs the POSIX installer and deterministic context checks on Linux, and the PowerShell installer suite on Windows. It runs for pull requests targeting `main`; the two job checks are required by default-branch protection. Containers can be added for distribution-specific portability testing, but they do not replace native Windows coverage.
